...
from partialObservability.ManualAI import ManualAI
from partialObservability.RandomAI import RandomAI
from partialObservability.MyAI import MyAI
from fullObservability.SearchAI import SearchAI
import random
...
class World:
    ...
    def __init__ ( self, debug = False, randomAI = False, manualAI = False, search = False, worldFile = None ):

        # Operation Variables
        self.__debug        = debug      # If True, displays board info after every move
        self.__manualAI     = manualAI   # If True, alters the behavior of debug for flow purposes
        self.__search       = search     # If True, we are in total observable case, passes 'board' to the agent!

        # Agent Variables
        self.__score        = 0          # The agent's score
        self.__goldLooted   = False      # True if gold was successfully looted
        self.__hasArrow     = True       # True if the agent can shoot
        self.__bump         = False      # Bump percept flag
        self.__scream       = False      # Scream percept flag
        self.__agentDir     = 0          # The direction the agent is facing: 0 - right, 1 - down, 2 - left, 3 - up
        self.__agentX       = 0          # The column where the agent is located ( x-coord = col-coord )
        self.__agentY       = 0          # The row where the agent is located ( y-coord = row-coord )

        self.__lastAction   = Agent.Action.CLIMB  # The last action the agent made

        if worldFile is not None:
            self.__colDimension, self.__rowDimension = [int(x) for x in next(worldFile).split()]
            self.__board = [[self.Tile() for j in range(self.__rowDimension)] for i in range(self.__colDimension)]
            self.__addFeatures(worldFile)
        else:
            self.__colDimension = 4
            self.__rowDimension = 4
            self.__board = [[self.Tile() for j in range(self.__colDimension)] for i in range(self.__rowDimension)]
            self.__addFeatures()
        # Agent
        if search:
            #Fix me
            if manualAI:
                ...
                self.__agent = SearchAI(self.__board)
        else:
            if randomAI:
    ...
    def __printTileInfo ( self, c, r ):
        ...
        if self.__board[c][r].gold:   tileString += "$"
        if self.__board[c][r].breeze: tileString += "~"
